...
--
-- Stand-in structuur voor view `vw_sales`
...
--
-- Structuur voor de view `vw_sales`
--
DROP TABLE IF EXISTS `vw_sales`;

CREATE ALGORITHM=UNDEFINED DEFINER=`qndrs_qndrs`@`localhost` SQL SECURITY DEFINER VIEW `vw_sales`  AS SELECT `products`.`productLine` AS `productLine`, year(`orders`.`orderDate`) AS `orderYear`, sum((`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`)) AS `orderValue` FROM ((`orderdetails` join `orders` on((`orderdetails`.`orderNumber` = `orders`.`orderNumber`))) join `products` on((`orderdetails`.`productCode` = `products`.`productCode`))) GROUP BY `products`.`productLine`, year(`orders`.`orderDate`) ;
...
--
-- Indexen voor tabel `productlines`
--
